...
//	struct rte_eth_conf port_conf;
	const uint16_t rx_rings = 10, tx_rings = 1;
				...
				if (ipv4->next_proto_id == 6)
				{
					...
					for (int i = 0; i < Len; i++) // 需要拷贝的字节数
					{
						...
						if (buckets[ret].limit - 1 < packet) // 需要几个数据包
						{
							...
							// printf("flow %d %f\n", ret, fe[ret].n[buckets[ret].limit - 1][i]);
						}
					}
					...
					cycle_diff += end_time - start_time;
...
//	rte_eal_remote_launch(lcore_main, NULL, 2);
	...
	while (1)
	{
		...
		double time_in_sec = 1000 * (double)cycle_diff / rte_get_timer_hz();

		printf("程序运行时间：%d %f ms\n",test[0] ,time_in_sec + totalMilliseconds);
	}
