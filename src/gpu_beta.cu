...
//	printf("Num%d %d\n",idx,d_a[idx].id);
	...
	uint16_t hash_idx = (dst_addr_bytes[0]+ dst_addr_bytes[1]+dst_addr_bytes[2]+dst_addr_bytes[3])*sport_host/dport_host;
}
else if (ipv4->next_proto_id == 17)
{
//	struct rte_tcp_hdr *udp = (struct rte_udp_hdr *)(ipv4 + 1);
